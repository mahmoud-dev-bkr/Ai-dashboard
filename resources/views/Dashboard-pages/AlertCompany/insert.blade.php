...
<div class="p-7">
    ...
    <div class="grid items-center w-full grid-cols-3 my-2 md:w-9/12 ">
        <label class="block w-40">Company Name</label>
        <select  name="company[]" id="company" class="js-select2 select select-bordered bg-base-300/50 w-80">
            @foreach ( $company as $com )    
                <option value="{{$com->id}}">{{$com->name_en}}</option>
            @endforeach
        </select>         
    ...
    <div class="grid items-center w-full grid-cols-3 my-2 md:w-9/12 ">
        <label class="block w-40">Select Message</label>
        <select  name="alert[]" id="msg" class="js-select2 select select-bordered bg-base-300/50 w-80">
            @foreach ( $message as $msg )    
                <option value="{{$msg->id}}">{{$msg->message_en}}</option>
            @endforeach
        </select>         
    ...
    <button type="submit" data-mdb-ripple="true" data-mdb-ripple-color="light"
        class="inline-block px-6 py-2.5 btnbg text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Send
        New Message</button>
    {!! Form::close() !!}
</div>
@endsection
...
>>>>>>> 0fdafc770b0fa472b6453bd28ef04d1c2c49f5e5
        $(document).ready(function() {
            // ////////////////////////////
