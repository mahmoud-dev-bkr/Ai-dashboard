...
@section('content')
    <div class="p-5 md:p-10">
        ...
        {!! Form::open(['route' => 'editfaq', 'class' => 'form-UPfaq', 'enctype' => 'multipart/form-data']) !!}
        <input type="hidden" name="id" value="{{ $faq->id }}">
        <div class="items-center my-2 input-group">
            <label class="font-bold w-80">Question (en)</label>
            <input type="text" value="{{ $faq->title }}" class="w-full input bg-base-300/50" name="title_en" />
        </div>

        <div class="items-center my-2 input-group">
            <label class="font-bold w-80">Answer (en)</label>
            <textarea name="body_en" class="w-full textarea h-80 bg-base-300/50 arabic">{{ $faq->paragraph }}</textarea>
        </div>


        <div class="items-center my-2 input-group">
            <label class="font-bold w-80">Question (ar)</label>
            <input type="text" value="{{ $faq->title_ar }}" class="w-full input bg-base-300/50 arabic" name="title_ar" />
        </div>

        <div class="items-center my-2 input-group">
            <label class="font-bold w-80">Answer (ar)</label>
            <textarea name="body_ar"
                class="w-full textarea h-80 bg-base-300/50 arabic">{{ $faq->paragraph_ar }}</textarea>
        </div>
