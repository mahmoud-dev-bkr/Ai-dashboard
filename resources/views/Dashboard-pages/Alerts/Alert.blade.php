...
@section('content')
    <div class="overflow-x-auto p-7">
        ...
        @if (Auth::user()->hasPermission('alerts_edit'))
            <div class="my-10">
                <a href="{{ LaravelLocalization::localizeUrl(route('insertalertPage')) }}"
                    class="rounded-full btn btn-info"><i class="fa fa-plus"></i></a>
                <span class="mx-3 text-lg font-bold">create a new Alert</span>
            </div>
        @endif
        @if (Auth::user()->hasPermission('alerts_view'))
            <table class="table w-full my-4 table-zebra" id="alertDT">
                <thead>
                    <tr>
                        ...
                        <th>Modify Data</th>
                        <th>Message Type</th>
                        @if (Auth::user()->hasPermission('alerts_activate'))
                            <th>Activate</th>
                        @endif
                        @if (Auth::user()->hasPermission('alerts_edit'))
                            <th>Edit</th>
                        @endif
                        @if (Auth::user()->hasPermission('alerts_delete'))
                            <th>Delete</th>
                        @endif
...
@section('scripts')
    <script>
        ...
        const toggleactivate = (id, activeState) => {
            ...
            (async () => {
                try {
                    const rawResponse = await fetch('{{ route('togglealertactivate') }}', {
                        method: 'PATCH',
                        headers: {
